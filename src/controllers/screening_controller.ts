import { Screening, IScreening } from '../models/screening';
import { Hall, IHall } from '../models/hall';
import { IError } from '../utils/error';
import { Request, Response } from "express";

export const screeningsGet = (req: Request, res: Response): void => {
...
export const screeningPost = (req: Request, res: Response): Response => {
    ...
    const newScreening: IScreening = new Screening({
        film: req.body.film,
    ...
    Hall.findById(req.params.hall_id)
        .populate('screenings')
        .then((doc: IHall): Promise<[IHall, IScreening] | IError> => {
            if (!doc) {
        ...
        .then((result: [IHall, IScreening] | IError) => {
...
export const screeningGet = (req: Request, res: Response): void => {
...
export const screeningPut = (req: Request, res: Response): Response => {
    if (!(req.body.film && req.body.beginning && req.body.end)) {
        return res.send(`Couldn't save screening - need all data about screening to save.`)
    }
    Screening.findById(req.params.screening_id)
        .then((doc: IScreening): Promise<[IHall, IHall, IScreening] | IHall | IScreening | IError> => {
            if (!doc) {
                ...
                if (req.body.hall) {
                    ...
                    ]).then((halls: [IHall, IHall]): Promise<IScreening | [IHall, IHall, IScreening] | IError> => {
        ...
        .then((result: [IHall, IHall, IScreening] | IHall | IScreening | IError) => {
...
export const screeningDelete = (req: Request, res: Response): void => {
    Screening.findByIdAndDelete(req.params.screening_id)
        .then((doc: IScreening): void => {
            if (!doc) {
...
function screeningsDontOverlap(doc: IHall, newScreening: IScreening) {
    return doc.screenings.every((screening: IScreening) => screening.beginning >= newScreening.end || screening.end <= newScreening.beginning);
}